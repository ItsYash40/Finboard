...
export const analyticsPromise = isAnalyticsSupported().then((supported) => (supported ? getAnalytics(app) : null));

let activeRecaptchaContainerId = null;

export function getRecaptchaVerifier(containerId) {
  if (window.recaptchaVerifier && activeRecaptchaContainerId === containerId) {
